...
namespace Config {
    ...
    void init_config()
    {
        ...
        if(settings.value("HistoryCapacity", -1) == -1) {
            settings.setValue("HistoryCapacity", 50);
        }
        ...
        //Columns
        if(settings.value("Columns/Id", -1) == -1) {
            settings.setValue("Columns/Id", true);
        }
    ...
    {
        COLLUMNS columns;
        columns.Id = settings.value("Columns/Id").toBool();
    ...
    {
        settings.setValue("Columns/Id", columns.Id);
    ...
    {
        return settings.value("HistoryCapacity", 50).toUInt();
    }

    void setHistoryCapacity(const unsigned int &size)
    {
        settings.setValue("HistoryCapacity", size);
    }
}
