...
bool PlaylistCache::createDatabase()
{
	if(QSqlDatabase::contains(dbConectionName))
	{
...
QSqlDatabase PlaylistCache::database()
{
	if(!QSqlDatabase::contains(dbConectionName))
	{
...
bool PlaylistCache::update(const SONG_METADATA &metadata)
{
	if(!open())
	{
		return false;
...
bool PlaylistCache::update(const std::vector<SONG_METADATA> &metadata)
{
	if(!open())
	{
		return false;
...
bool PlaylistCache::updateInternal(const SONG_METADATA &metadata)
{
...
std::optional<SONG_METADATA> PlaylistCache::getSongInternal(const QString &filePath)
{
	auto db = database();
	QSqlQuery q(db);
...
std::optional<SONG_METADATA> PlaylistCache::getSong(const QString &filePath)
{
	open();
