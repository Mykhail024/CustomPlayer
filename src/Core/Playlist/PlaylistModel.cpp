...
PlaylistModel::PlaylistModel(const QString &filePath, Cache *cache, QObject *parent)
...
SONG_METADATA PlaylistModel::getSongMetadata(const size_t &index) const
{
    if(index > m_songs.size() - 1 || index < 0) {
        Log_Warning(QString("No Song on index %1 in playlist %2 (%3)").arg(index).arg(m_name).arg(m_filePath));
        return {};
    }
